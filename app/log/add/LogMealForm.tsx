...
export default function LogMealForm({ recipes }: { recipes: Recipe[] }) {
  ...
  async function save() {
    ...
    try {
      const res = await fetch("/api/logs/quick", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
        ...
        rating: rating || null, notes, cookedAt: date, countsAsCooked, context, venue, link, tags: tags.split(",").map((tag) => tag.trim()).filter(Boolean), photo,
      }) });
  ...
  return <div className={styles.wrap}>
    ...
    <section className={`card ${styles.form}`}>
      ...
      {!recipeId && <label>What was it?<input className="input" value={title} onChange={(event) => setTitle(event.target.value)} placeholder={context === "out" ? "e.g. Smoky prawn tacos" : "e.g. Tuesday night curry"} /><VoiceDictation onTranscript={(text) => setTitle((current) => `${current}${current ? " " : ""}${text}`)} /></label>}
      ...
      <div className={styles.photoAction}><button type="button" className="btn btn-secondary" onClick={() => cameraPhotoRef.current?.click()}>📷 Take a photo</button><button type="button" className="btn btn-secondary" onClick={() => libraryPhotoRef.current?.click()}>🖼 Choose from library</button>{photo && <><img src={photo.preview} alt="Meal preview" /><button type="button" className={styles.remove} onClick={() => { setPhoto(null); setCountsAsCooked(true); }}>Remove</button></>}{reading && <span className={styles.reading}>🔎 Reading photo…</span>}</div>
      {photo && <label className={styles.retrospective}><input type="checkbox" checked={!countsAsCooked} onChange={(event) => setCountsAsCooked(!event.target.checked)} /><span><strong>This is a past meal</strong><small>Save the photo and memory, but don&rsquo;t count it as cooked now.</small></span></label>}
      ...
      {!recipeId && <><label>Link <input className="input" type="url" value={link} onChange={(event) => setLink(event.target.value)} placeholder="Instagram, TikTok, article, or recipe link" /></label><button type="button" className={styles.pasteToggle} onClick={() => setPasteOpen((open) => !open)}>📋 {pasteOpen ? "Hide pasted recipe" : "Paste recipe text"}</button>{pasteOpen && <div className={styles.paste}><textarea className="input" rows={6} value={paste} onChange={(event) => setPaste(event.target.value)} placeholder="Paste a recipe, menu description, or a note from your holiday…" /><VoiceDictation onTranscript={(text) => setPaste((current) => `${current}${current ? " " : ""}${text}`)} /><button type="button" className="btn btn-secondary" disabled={sorting} onClick={sortPaste}>{sorting ? "Sorting…" : "✨ Fill in details"}</button></div>}<details open={detailsOpen} onToggle={(event) => setDetailsOpen(event.currentTarget.open)}><summary>Recipe details (optional)</summary><label>Ingredients<textarea className="input" rows={3} value={ingredients} onChange={(event) => setIngredients(event.target.value)} /><VoiceDictation onTranscript={(text) => setIngredients((current) => `${current}${current ? "\n" : ""}${text}`)} /></label><label>Method<textarea className="input" rows={4} value={instructions} onChange={(event) => setInstructions(event.target.value)} /><VoiceDictation onTranscript={(text) => setInstructions((current) => `${current}${current ? "\n" : ""}${text}`)} /></label></details></>}
      ...
      <label>Notes<textarea className="input" rows={3} value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Who was there? What would you make differently?" /><VoiceDictation onTranscript={(text) => setNotes((current) => `${current}${current ? " " : ""}${text}`)} /></label>
